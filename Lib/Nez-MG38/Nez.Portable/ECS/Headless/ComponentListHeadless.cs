...
{
	public class ComponentListHeadless
	{
		// global updateOrder sort for the IUpdatable list
		static IUpdatableComparer compareUpdatableOrder = new IUpdatableComparer();

		EntityHeadless _entity;

		FastList<ComponentHeadless> _components = new FastList<ComponentHeadless>();
		...
		/// </summary>
		internal List<ComponentHeadless> _componentsToAdd = new List<ComponentHeadless>();
		...
		/// </summary>
		List<ComponentHeadless> _componentsToRemove = new List<ComponentHeadless>();

		List<ComponentHeadless> _tempBufferList = new List<ComponentHeadless>();
		...
		public ComponentListHeadless(EntityHeadless entity)
		{
			_entity = entity;
		}
		...
		public ComponentHeadless this[int index] => _components.Buffer[index];
		...
		public void Add(ComponentHeadless component)
		{
			_componentsToAdd.Add(component);
		}

		public void Remove(ComponentHeadless component)
		{
		...
		void HandleRemove(ComponentHeadless component)
		{
			// deal with IUpdatable
			if (component is IUpdatable)
				_updatableComponents.Remove(component as IUpdatable);
		...
		internal void Update()
		{
			...
			{
				if (_updatableComponents.Buffer[i].Enabled && (_updatableComponents.Buffer[i] as ComponentHeadless).Enabled)
					_updatableComponents.Buffer[i].Update();
			}
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		internal void OnEntityTransformChanged(TransformHeadless.Component comp)
		{
			for (var i = 0; i < _components.Length; i++)
			{
